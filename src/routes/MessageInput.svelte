<script>
  ...
  const MAX_SIZE = 1024 * 50; // 50KB limit
</script>
...
<div>
  <form
    ...
    on:submit|preventDefault={e => {
      ...
      if (messageSize > MAX_SIZE) {
        alert(`Message too large (${formatBytes(messageSize)}). Maximum size is ${formatBytes(MAX_SIZE)}.`);
        return;
      }
  ...
  >
    ...
    />
    {#if messageSize > 0}
      <div class="message-size" class:warning={messageSize > MAX_SIZE * 0.8} class:error={messageSize > MAX_SIZE}>
